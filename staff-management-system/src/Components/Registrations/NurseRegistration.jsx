...
const NurseRegistration = () => {
  ...
  const onSubmit = (data) => {
    ...
    // Construct the registration payload, excluding confirmPassword
    const registrationData = {
      firstName: data.firstName,
      lastName: data.lastName,
      _id: data.aadhaarNumber,
    ...
    <RegistrationContainer>
      <FormContainer>
        ...
        <Form onSubmit={handleSubmit(onSubmit)} noValidate>
          ...
          <FormSection>
            <SectionTitle>Skills & Description</SectionTitle>

            <InputGroup>
              <Input
                as="textarea"
                {...register("skills", { required: "Skills are required" })}
                placeholder="Skills (comma-separated, e.g., ICU, Pediatrics)"
                rows={4}
                hasError={!!errors.skills}
              />
              {errors.skills && <ErrorText>{errors.skills.message}</ErrorText>}
...
const InputGroup = styled.div`
  ...
  & textarea {
    font-family: inherit;
    padding: 0.75rem 1rem; /* Adjust padding for textareas */
    resize: vertical;
  }
`;
...
const InputIcon = styled.div`
  ...
  /* Adjust for inputs vs textareas */
  ${({ as }) =>
    as !== "textarea" &&
    `
    top: 50%;
